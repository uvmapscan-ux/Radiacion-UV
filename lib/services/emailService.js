...
function crearTransporte() {
  ...
  // --- DIAGNÓSTICO TEMPORAL: no expone valores, solo nombres de variables presentes ---
  const todasLasClaves = Object.keys(process.env);
  const relacionadas = todasLasClaves.filter(
    (k) => k.startsWith('SMTP_') || k.startsWith('EMAIL_') || k === 'CRON_SECRET'
  );
  logger.info(`DIAGNÓSTICO: total de variables de entorno visibles = ${todasLasClaves.length}`);
  logger.info(`DIAGNÓSTICO: variables SMTP_/EMAIL_/CRON_SECRET detectadas = [${relacionadas.join(', ')}]`);
